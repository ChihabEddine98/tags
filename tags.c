...
search_criteria_t tokenize(char **result, char *working, const char *src, const char *delim)
{
  ...
  for (k = 0; k < cpt; k++)
  {
    ...
    if (strstr(result[k], "Non") || strstr(result[k], "NON") || strstr(result[k], "non") || strstr(result[k], "Not") || strstr(result[k], "NOT") || strstr(result[k], "not") || strstr(result[k], "Pas") || strstr(result[k], "PAS") || strstr(result[k], "pas"))
    {
...
int main(int argc, char **argv)
{
  ...
  // MENU HERE !
  if (!arguments.rm_tag && !arguments.rm_category && !arguments.add_tag && !arguments.search_files_criteria && !arguments.search_tags_criteria && !arguments.add_category)
  {
    menu();
  }
